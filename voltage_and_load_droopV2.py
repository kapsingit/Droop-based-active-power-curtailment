...
def get_message(LOAD,time):
    ...
    for k in range(0,12):
        if voltage[k,0]>250:
            #if Pnet[k][time]>2000:
            A[k,0]=m*voltage[k,0]-m*V_cri[k,0]-Pcurtail[k,0]#+ q*(Pnet[k][time])
            #else:
            ...
            if PMPPT<-2000.0:                
                for ite_loop in range(0,25):
                    ...
                    for z,n in enumerate(node):
                        ...
                        if z == 11:
                            er.append(voltage[z,0]-voltage_prev)
                            voltage_prev = voltage[z,0]
                    
                        
                ermax_list.append(er)
...
ermax_list = pd.DataFrame(ermax_list)

ermax_list.to_csv('Error_list.csv')
